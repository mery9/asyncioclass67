...
async def create_topic():
    admin_client = AIOKafkaAdminClient(bootstrap_servers=['172.16.46.203:9092'])
    await admin_client.start()
    try:
        # Check if the topic exists, otherwise create it
        existing_topics = await admin_client.list_topics()
        if 'my_topic' not in existing_topics:
            # topic = NewTopic(name="my_topic", num_partitions=5, replication_factor=5)
...
async def consume():
    consumer = AIOKafkaConsumer(
        ...
        bootstrap_servers='172.16.46.203:9092'
        ,group_id="my-group"
        )
